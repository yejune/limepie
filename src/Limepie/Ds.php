...
class Ds implements \Iterator, \ArrayAccess, \Countable, \JsonSerializable // , \Serializable
{
    ...
    public function modify(array|string|int $key, mixed $value = null) : self
    {
        ...
        if (\is_array($key)) {
            ...
            $currentAttributes = \array_replace_recursive($currentAttributes, $key);

            return $this;
        }

        // $key와 $value를 사용하여 중첩된 배열 값을 덮어쓸 경우
        $currentAttributes = \array_replace_recursive($currentAttributes, [$key => $value]);
        $attributes        = \array_replace_recursive($attributes, [$key => $value]);
                ...
                if (0 === \strpos($key, 'getBy')) {
                    ...
                    if ($value) {
                        if (\is_array($value[0])) {
                            ...
                            // $key와 $value를 사용하여 중첩된 배열 값을 덮어쓸 경우
                            $currentAttributes = \array_replace_recursive($currentAttributes, [$key => $value[0]]);
                            $attributes        = \array_replace_recursive($attributes, [$key => $value[0]]);
                        }
                    }
...
// Di::getResponse()->meta2(Di::getResponse()->getMeta()->toArray());
// // \pr(Di::getResponse()->meta2());

// \pr(Di::getResponse()->meta2()->getServiceLayout()->getMenus()->getLeft()->getBreadcrumb()->remove()->current());

// Di::getResponse()->meta2()->getServiceLayout()->getMenus()->getLeft()->getBreadcrumb()->add([
//     'parent' => 0,
//     'seq'    => 3,
//     'name'   => $name,
//     'url'    => '/admin/seller/smartig-ticket',
//     'active' => 1,
//     'params' => [
//         'is_close' => 0,
//     ],
...
// Di::getResponse()->meta2()->getServiceLayout()->getMenus()->add('name', 'max');
// // \pr(Di::getResponse()->meta2()->getServiceLayout()->getMenus()->getName()->get());
// // \pr(Di::getResponse()->meta2()->getServiceLayout()->getMenus()->getLeft()->get());

// foreach (Di::getResponse()->meta2()->getServiceLayout()->getMenus()->getLeft()->getNavigation()->get() as $d) {
//     \pr($d, $d['seq'], $d->seq, $d->add('tmp', $d->seq + 10), $d->getBySeq(9999 + $d->seq)); // ));
// }

// // \pr(Di::getResponse()->meta2()->getC()->getD());
// \pr(Di::getResponse()->meta2());
