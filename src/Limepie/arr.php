...
class Arr
{
     ...
     *               예: ['a' => ['b' => ['c' => 'value']]]
     */
    public static function dot_to_nested(array $flatArray) : array
    {
        $result = [];
     ...
     */
    public static function split(array|ArrayObject $array, int $groups = 2) : array
    {
        if ($groups <= 0) {
    ...
    public static function getIsDisplay($serviceModuleBannerItem)
    {
        $isDisplay = false;
    ...
    public static function getDisplayStatus($serviceModuleBannerItem)
    {
        $now    = \date('Y-m-d H:i:s');
        $status = [
            'isDisplay'    => false,
     ...
     */
    public static function filter_recursive($array)
    {
        foreach ($array as $key => $value) {
            if (\is_array($value)) {
                $array[$key] = self::filter_recursive($value);
     ...
     */
    public static function insert_random($items, $newItem, $max = null)
    {
        // $items = [1, 2, 3, 4, 5];
        // $newItem = 6
        // 1 2 3 6 4 5
    ...
    public static function is_same($array1, $array2)
    {
        // 배열 크기 비교
    ...
    public static function key_value($array, $separator = ': ', $delimiter = ', ')
    {
        $result = [];
    ...
    public static function is_diff($array1, $array2)
    {
        return !self::is_same($array1, $array2);
    }

    // 5점이 3건, 4점이 2건, 3점이 1건이 있는 경우 로직상 가장 많은 것을 가득채우고 그것의 비례에 맞게 나머지를 비율로 채움
    public static function get_percent_stars($reviewCounts)
    {
    ...
    public static function replace($description, $row = [])
    {
    ...
    // 숫자 배열을 퍼센트로 변환
    public static function percent(array $numbers, $precision = 0)
    {
        $result = [];
     ...
     * @param mixed $in
     */
    public static function to_html(array $in, array $hidden_keys = ['password', 're_password']) : string
    {
        if (0 < \count($in)) {
            $t = '<div class="table-responsive"><table class="table table-sm table-bordered table-gray-bordered"><tbody>';

            foreach ($in as $key => $value) {
                if (true === self::is_assoc($in)) {
    ...
    public static function array_insert_before(&$array, $beforeKey, $newKey, $newValue)
    {
    ...
    public static function insert_before(array $array, $key, array|string $new)
    {
    ...
    public static function insert_after(array $array, $key, array|string $new)
    {
        if (false === \is_array($new)) {
            $new = [$new];
        }
    ...
    // public static function insert_before($key,&$array,$new_key,$new_value='NA'){
    //     if(array_key_exists($key,$array)){
    //         $new = array();
    ...
    // public static function insert_after($key,&$array,$new_key,$new_value='NA'){
    //     if(array_key_exists($key,$array)){
    //         $new = array();
    //         foreach($array as $k=>$value){
    //             $new[$k] = $value;
    ...
    public static function remove($result, $arr)
    {
    ...
    public static function merge_deep()
    {
        $args = \func_get_args();
    ...
    public static function key_flatten($array)
    {
        ...
        foreach ($array as $key => $value) {
            ...
            if (\is_array($value)) {
                $keys = \array_merge($keys, self::key_flatten($value));
            }
        }
    ...
    public static function value_flatten($array)
    {
        // if (!isset($array) || !\is_array($array)) {
        $values = [];
        // }
        ...
        foreach ($array as $key => $value) {
            if (\is_array($value)) {
                $values = \array_merge($values, self::value_flatten($value));
            } else {
                $values[] = $value;
            }
        }
    ...
    public static function flattenx($items)
    {
        if (!\is_array($items)) {
            return [$items];
        }
    ...
    public static function cross($arrays)
    {
        $result = [[]];
    ...
    public static function flatten_put($data, $flattenKey, $value)
    {
    ...
    public static function flatten_remove($data, $flattenKey)
    {
        $items       = &$data;
    ...
    public static function change_key_case_recursive(array $arr, int $case = \CASE_LOWER)
    {
        return \array_map(function ($item) use ($case) {
            if (true === \is_array($item)) {
                $item = self::change_key_case_recursive($item, $case);
            }
    ...
    public static function reverse(array|ArrayObject $array = []) : array
    {
    ...
    public static function value_sum(array $array) : int
    {
        return \array_sum(\array_values($array));
    }

    public static function _set(array|ArrayObject $array, array|\Closure|string $params)
    {
    ...
    public static function _unset(array|ArrayObject $array, array|string $params)
    {
        if (true === \is_string($params)) {
            $params = [$params];
        }
    ...
    public static function set(array|ArrayObject $array, array|string $params)
    {
        if (true === \is_string($params)) {
            $params = [$params];
        }
    ...
    // array_unset -> delete
    public static function delete(array|ArrayObject $array, array|string $params)
    {
        if (true === \is_string($params)) {
            $params = [$params];
        }
    ...
    public static function key_rename(array $array, $old_key, $new_key)
    {
    ...
    public static function cleanup(array|ArrayObject $value) : ?array
    {
    ...
    public static function cleanup2(array|ArrayObject $array)
    {
        if ($array instanceof ArrayObject) {
            $array = $array->attributes;
        }
    ...
    public static function values_pick_keys($array, $key, $keepKey = false)
    {
        $return = [];
        $index  = -1;
    ...
    public static function pick_keys($array, $key)
    {
        $return = [];
    ...
    public static function tree1($array, $parent = 0)
    {
        $ret = [];
        ...
        for ($i = 0; $i < \count($array); ++$i) {
            if ($array[$i]['parent_seq'] === $parent) {
                ...
                $a['item'] = self::tree1($array, $a['current_seq']);
                $ret[]     = $a;
    ...
    public static function tree2($array, $parent = 0)
    {
        $ret = [];
        ...
        foreach ($array as $i => $a) {
            ...
            if ($a['parent_seq'] == $parent) {
                $sub = self::tree2($array, $a['current_seq']);
    ...
    public static function injection_key($datasets, $requiredKeys)
    {
    ...
    public static function flatten_get($data, $flattenKey)
    {
        $keys = \explode('[', \str_replace(']', '', $flattenKey));
    ...
    public static function array_rmerge($arr1, $arr2)
    {
        $merged = $arr1;

        foreach ($arr2 as $key => &$value) {
            if (\is_array($value) && isset($merged[$key]) && \is_array($merged[$key])) {
                $merged[$key] = self::array_rmerge($merged[$key], $value);
            } else {
                $merged[$key] = $value;
            }
        }
        ...
        foreach ($arrays as $array) {
            foreach ($array as $key => $value) {
                ...
                if (true === \is_int($key)) {
                    ...
                    $result[$key] = self::drupal_array_merge_deep_array([
                        $result[$key],
                        $value,
                    ]);
                } else {
     ...
     * env to array.
     */
    public static function env_to_array(string $envPath) : array
    {
        $variables = [];
     ...
     * @param bool  $isMulti
     */
    public static function is_file_array($array = [], $isMulti = false) : bool
    {
        if (true === \is_array($array)) {
            if (
    ...
    public static function file_array_flatten($list, $prefix = '')
    {
        $result = [];
        ...
        foreach ($list as $name => $value) {
            if (true === \is_array($value)) {
                ...
                if (true === self::is_file_array($value, false)) {
                    $result[$newPrefix] = $value;
                } else {
                    $result += self::file_array_flatten($value, $newPrefix);
                }
            }
        }
    ...
    public static function nest(array $flat, $value = []) : array
    {
        if (!$flat) {
            return $value;
        }
        ...
        return [$key => self::nest($flat, $value)];
    }
    ...
    public static function flatten_diff($arraya, $arrayb)
    {
        $old = $arraya = self::flatten($arraya);
        $new = $arrayb = self::flatten($arrayb);
    ...
    public static function xml2array(\SimpleXMLElement $xml) : array
    {
     ...
     * @param mixed $isAll
     */
    public static function extractKeys(?array $array, array|string $keysToExtract, $isAll = true)
    {
        if (null === $array) {
            return null;
        }
     ...
     * @return array 정렬된 배열
     */
    public static function ksort($array, $keys)
    {
        // 결과를 저장할 배열
        $result = [];
    ...
    public static function key_sort(&$array, $path)
    {
        $paths = \explode('.', $path);
     ...
     */
    public static function refparse($arr = [], $basepath = '') : array
    {
    ...
    public static function legacy_refparse($arr = [], $basepath = '') : array
    {
        $return = [];
        ...
        foreach ($arr ?? [] as $key => $value) {
            if ('$ref' === $key) {
                ...
                foreach ($value as $path) {
                    ...
                    if ($path) {
                        ...
                        if ($referenceData) {
                            ...
                            $newData = self::legacy_refparse($data, $basepath);
                ...
                foreach ($value as $k => $v) {
                    foreach ($v as $v1) {
                        $return = self::insert_after($return, $k, $v1);
                    }
                }
            } elseif ('$before' === $key) {
                foreach ($value as $k => $v) {
                    foreach ($v as $v1) {
                        $return = self::insert_before($return, $k, $v1);
                    }
                }
            } elseif ('$change' === $key) {
                foreach ($value as $k => $v) {
                    if (true === isset($return[$k])) {
                        $return[$k] = self::merge_deep($return[$k], $v);
                    } else {
                ...
                if (true === \is_array($value)) {
                    foreach ($value as $k => $v) {
                        if (true === isset($return[$k])) {
                            ...
                            */
                            if (true === \is_array($v)) {
                                $return[$k] = self::remove($return[$k], $v);
                            } else {
                                unset($return[$k]);
                            }
                ...
                if (true === \is_array($value)) {
                    if (true === isset($value['lang']) && 1 === \preg_match('#\[\]$#', $key, $m)) {
                        ...
                        $default['properties'][$langKey] = self::legacy_refparse($default['properties'][$langKey], $basepath);

                        $return[$key] = self::legacy_refparse($default, $basepath);
                        // \prx($key, $return[$key]);
                        ...
                        if ('append' === $value['lang']) {
                            ...
                            $return[$key] = self::legacy_refparse($value, $basepath);
                        }
                                ...
                                foreach ($value['langs'] as $langKey => $langProperty) {
                                    if (isset($langProperties[$langKey])) {
                                        if ($langProperty) {
                                            ...
                                            $newLangProperties[$langKey] = self::merge_deep($langProperties[$langKey], $langProperty);
                                        } else {
                        ...
                        $return[$key . '_langs'] = self::legacy_refparse($value, $basepath);
                    } else {
                        $return[$key] = self::legacy_refparse($value, $basepath);
                    }
                } else {
    ...
    public static function extract_property($array, $propertyName = 'name')
    {
        $names = [];
