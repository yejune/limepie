    ...
    public function getRelationData(Model $class, $attribute, $functionName, $parentTableName = '', $isSingle = true)
    {
        ...
        // 부모 테이블이 없는 경우
            ...
            // 그 외의 경우
                ...
                //             if (!$value->sub) {
                //                 $value->sub            = 3;
                //                 $fetchAttributes[$key] = new $class($connect, ($class->valueName)($value), $value->attributes, true);
                //             }
                //         }
                //     }
                //     // }
    ...
    public function executeGet(string $sql, array $binds = [])
    {
        ...
        if ($attributes) {
            ...
            if ($this->valueName instanceof \Closure) {
                $this->sub        = 5;
                $this->attributes = ($this->valueName)($this);
            }
                ...
                foreach ($attributes as $key => $attribute) {
                    if ($attribute instanceof self) {
                        if (!$attributes[$key]->sub) {
                            $attributes[$key]->sub        = 2;
                            $attributes[$key]->attributes = ($this->valueName)($attributes[$key]);
                        }
                    }
                }
                // \prx($attributes);
