...
class Model extends ArrayObject
{
    ...
    {
        $orgKey   = $class->keyName;
        $seqs     = [];
        $data     = [];
        $remapKey = $remapKey = $class->keyName;
        ...
        // single일 경우는 right로 바꿔야 한다. 어차피 중복이 없기 때문이다.
        ...
        if ($isSingle) {
            ...
            foreach ($attributes as $attribute) {
                ...
                // \pr($attribute, $leftKeyName, $rightKeyName, $attr, $data, $group);
            ...
            if (false === \in_array($alias, $this->removeColumns, true)) {
                ...
                if (true === \is_numeric($column)) {
                    if ('ip' === $alias) {
                        ...
                        if (false === $isCount) {
                            $this->binds[':' . $prefix . $alias . '_secretkey' . $rand . ''] = \Limepie\Aes::$salt;
                        }
                    } elseif (
                    ...
                    if (true === \is_object($alias)) {
                        ...
                        if ($alias->format) {
                            ...
                            if (isset($this->dataStyles[$alias->columnName]) && 'aes_hex' === $this->dataStyles[$alias->columnName]) {
                                ...
                                if (false === $isCount) {
                                    $this->binds[':' . $prefix . $alias->aliasName . '_secretkey' . $rand . ''] = \Limepie\Aes::$salt;
                                }
        ...
        foreach ($data as $index => &$row) {
            ...
            if ($keyName) {
                if (false === \array_key_exists($this->keyName, $row)) {
                    ...
                    throw new \Limepie\Exception('gets by ' . $this->tableName . ' "' . $keyName . '" column not found #7');
                }
