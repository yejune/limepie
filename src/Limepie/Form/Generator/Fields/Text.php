...
class Text extends \Limepie\Form\Generator\Fields
{
    public static function write($key, $property, $value, $ruleName, $propertyName)
    {
        ...
        return <<<EOT
            ...
            {$prepend}
            <input type="text" class="valid-target form-control{$elementClass}" name="{$key}" value="{$value}" data-name="{$propertyName}" data-rule-name="{$ruleName}"  data-default="{$default}"{$readonly}{$disabled}{$placeholder}{$style} />
            {$append}
            </div>
        EOT;
    }
