...
class Select2 extends \Limepie\Form\Generator\Fields
{
...
<script src="https://cdn.jsdelivr.net/npm/bootstrap-select@1.14.0-beta3/dist/js/i18n/defaults-*.min.js"></script>
        ...
        {$prepend}
        <select class="valid-target form-control{$class}" {$style} name="{$key}" data-name="{$propertyName}" data-rule-name="{$ruleName}"  {$onchange} data-default="{$default}">{$option}</select>
        {$append}
        </div>
        {$scripts}
EOT;
